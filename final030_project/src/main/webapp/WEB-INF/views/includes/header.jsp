		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="right-phone-box">
					<p>
						입점 문의 : <a href="#">+82-10-8824-3282</a>
					</p>
				</div>
				<div class="our-link">
					<ul>
				...
				<c:if test="${loginParty == null}">
					<div id="btn_group">
						<form action="/party/regist">
							<button type="submit" id="test_btn1">회원가입</button>
						</form>
						<form action="/party/login">
							<button type="submit" id="test_btn2">로그인</button>
						</form>
					</div>
				</c:if>
				<!-- 로그인한 상태 -->
				<c:if test="${loginParty != null}">
					<div class="login-box" style="float: right;">
						<div class="login_success_area">
							<span style="padding-right: 10px;">아이디 : ${loginParty.userId}</span>
							<span>회원 : ${loginParty.userName}</span><br>
							<span hidden="hidden">권한 : ${loginParty.authority}</span>
							<a href="/party/logout" style="color: silver; float: right;">로그아웃</a>
						</div>
					</div>
				</c:if>
			</div>
	...
	<nav
		...
		<div class="container">
			...
			<div class="menu">
				<ul>
					<li><a href='/'>Home</a></li>
					<li class='sub'><a href='#'>지역 카테고리</a>
						<ul id="locationCategory">
							<c:forEach items="${locationList}" var="location">
								<li class='sub'><a name="${location.locationId}">${location.locationName}</a>
									<ul>
									</ul></li>
							</c:forEach>
						</ul>
					...
					<c:if test="${loginParty.authority eq 'seller'}">
						<li><a href='/post/startLive'>Live</a></li>
					</c:if>

					<c:if test="${loginParty.authority ne 'seller'}">
						<li><a href='/post/watchLive'>Live</a></li>
					</c:if>
					...
					<c:if test="${loginParty != null}">
						<li><a href='/party/purchase'>Purchase</a>
						</li>
					</c:if>

					<c:if test="${loginParty.authority eq 'seller'}">
						<li><a href='/post/managementProduct?boardId=5'>상품관리</a></li>
					</c:if>

					<c:if test="${loginParty.authority eq 'admin'}">
						<li class='last'><a href='/post/postList?boardId=6'>관리자페이지</a>
						</li>
					</c:if>
...
<form id="frmLocSearch" action="/post/searchingProductList"
	method="get">
