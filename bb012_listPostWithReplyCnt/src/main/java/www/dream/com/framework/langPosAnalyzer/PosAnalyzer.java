...
/**
 * 품사 분석기가 정의한 Annotation을 달아 놓은 객체의 속성에 들어있는 정보를 Komoran을 활용하여 품사 분석하고 해쉬태그로
 * 활용할 만한 단어가 몇 번 사용되었는지 까지를 ComparablePair의 List로 반환할 것 입니다.
 */
public class PosAnalyzer {
	private static Komoran komoran = new Komoran(DEFAULT_MODEL.FULL);
	
	/** 주어진 객체에서 각종 속성 및 함수 위에 @HashTarget를 달아서 노출시킨 정보를 바탕으로 
		...
		for (Token token : tokenList) {
			...
			if (pos != null) {
				...
				if (ret.containsKey(hashTag)) {
					ret.put(hashTag, (ret.get(hashTag) + 1)); // 값이 있으면 1씩 추가하고 아니면 1로 고정
				} else {
					ret.put(hashTag, 1);
				}
			}
